...
> **底层**
>
> **CGLIB 包的底层是通过使用一个小而快的字节码处理框架 ASM (Java字节码操控框架)，来转换字节码并生成新的类。**除了 CGLIB包，脚本语言例如 Groovy 和 BeanShell，也是使用 ASM 来生成 java 的字节码。**不鼓励直接使用 ASM，因为它要求你必须对JVM 内部结构包括 class 文件的格式和指令集都很熟悉。**所以 cglib 包要依赖于 asm 包，需要一起导入
...
>         System.out.println("新增书本" + name);
>     }
> 
>     @Log
>     public int edit(int id, String name) {
>         System.out.println("修改书本" + name + "id是" + id);
>         return 1;
>     }
> 
>     public int del(int id) {
>         System.out.println("删除书本" + "id是" + id);
>         return 1;
>     }
> }
> ```
...
>     public void add2Car(int userid, int bookid) {
>         System.out.println("添加到购物车" + userid + "-->" + bookid);s
>     }
> 
>     @Ex("清空购物车失败")
>     public void clearCar(int userid) {
>         System.out.println("清空购物车" + userid);
>     }
> 
>     public int total(int userid) {
>         System.out.println("统计用户购物车总数" + userid);
>         return 10;
>     }
> }
> ```
...
> @Target(ElementType.METHOD) // 可以方法上使用
...
> // 指定要生成谁的子类(编织代码进去)
> en.setSuperclass(BookServiceImpl.class);
> en.setCallback(new MethodInterceptor() {
>     @Override
>     public Object intercept(Object obj, Method method, Object[] objects, MethodProxy methodProxy) throws Throwable {
>         Log gg = method.getAnnotation(Log.class);
>         // 判断一下有没有注解Log
>         if (gg != null)
>             System.out.println("日志：开始操作" + method.getDeclaringClass().getSimpleName() + "的" + method.getName() + "方法");
>         // 调用原装的方法(BookServiceImpl中的 add，edit，del)
>         Object o = methodProxy.invokeSuper(obj, objects);
>         if (gg != null)
>             System.out.println("日志：操作完成" + method.getDeclaringClass().getSimpleName() + "的" + method.getName() + "方法");
>         return o;
>     }
> });
> // 配置完毕，创建子类
> BookServiceImpl service2 = (BookServiceImpl) en.create(); // 自动生成子类，为里面的每个方法都进行代码增强
> service2.add("窗边的小豆豆");
> service2.edit(1, "窗边的小豆豆");
> service2.del(1);
> ```
>
> ==intercept方法的参数说明==
...
> - method 自动生成的子类的方法
> - objects 方法的参数列表
> - methodProxy 代理的方法(调用的父类的方法)
>
> *代理 CarServiceImpl*
>
> ```java
> Enhancer en = new Enhancer();
> en.setSuperclass(CarServiceImpl.class);
> en.setCallback((MethodInterceptor) (obj, method, objects, methodProxy) -> {
>     Ex e = method.getAnnotation(Ex.class);
>     // 判断一下有没有注解Ex
>     if (e == null)
...
>         ex.printStackTrace();
>         throw new RuntimeException(e.value() + ex);
>     } finally {
>         System.out.println("finally");
>     }
> });
> CarServiceImpl service = (CarServiceImpl) en.create();
> service.add2Car(1, 1);
> service.clearCar(1);
> service.total(1);
> ```
...
>        public static Object createCglibProxy(Class c, MethodInterceptor interceptor) {
>            Enhancer en = new Enhancer();
>            en.setSuperclass(c); // 指定要生成谁的子类(编织代码进去)
...
>
> ```java
> public class CglibLogAop implements MethodInterceptor {
>        @Override
>        public Object intercept(Object obj, Method method, Object[] objects, MethodProxy methodProxy) throws Throwable {
>            // 这个方法有没有写Log注解
>            Log gg = method.getAnnotation(Log.class);
...
>
> ```java
> BookServiceImpl proxy = (BookServiceImpl) CglibUtil.createCglibProxy(BookServiceImpl.class,new CglibLogAop());
> proxy.add("窗边的小豆豆");
> proxy.edit(1, "窗边的小豆豆");
> proxy.del(1);
> ```
...
> **动态代理的原理**
>
> 与 cglib 代理的继承关系不同，JDK 动态代理中的原始类和代理类之间没有直接的关系。
>
> 要让原始类和代理类拥有相同的方法实现代理，那么最好的方式就是实现相同的接口
>
> ![][6]
>
> *必须要有接口才能实现 JDK 动态代理(否则只能使用 cglib 代理)*
>
> **动态代理步骤**
>
...
>
> ```java
> Subject subject = (Subject) Proxy.newProxyInstance(loader, interfases, handler)
> ```
>
> - loader：类加载器(随便找一个本工程的类即可)
> - interfases：想代理的接口的数组
> - handler：调用的方法
>
> **示例**
>
...
>     @Log
>     @Override
>     public void add(String name) { System.out.println("新增书本" + name); }
>     
>     @Override
>     public int edit(int id, String name) {
>         System.out.println("修改书本" + name + "id是" + id);
...
>     StartSys.class.getClassLoader(), // 类加载器(随便找一个本工程的类即可)
>     new Class[]{BookService.class}, // 想代理的接口类型
>     new InvocationHandler() {
>         BookService service = new BookServiceImpl(); // 创建原始对象
>         @Override
...
>        public static Object createJdkProxy(Class face, InvocationHandler handler) {
>            return createJdkProxy(new Class[]{face}, handler);
>        }
> 
>        public static Object createJdkProxy(Class[] interfaces, InvocationHandler handler) {
>            Object proxy = Proxy.newProxyInstance(ProxyUtil.class.getClassLoader(), interfaces, handler);
>            return proxy;
>        }
> }
> ```
...
>
> ```java
> public class JdkLogAop implements InvocationHandler {
>        private Object srcObj = null;
> 
>        public JdkLogAop(Class c) {
>            try {
>                // 传入一个类型，用反射的方式创建对象
>                srcObj = c.newInstance();
>            } catch (Exception e) {
...
>        public Object invoke(Object obj, Method method, Object[] args) throws Throwable {
>            // 获得原始对象接口的实现类的方法
>            Method m = srcObj.getClass().getMethod(method.getName(), method.getParameterTypes());
>            // 判断一下实现类中的方法上有没有注解
>            Log log = m.getAnnotation(Log.class);
>            // 也判断一下接口上有没有注解
>            if (log == null)
...
>            System.out.println("日志：开始操作" + srcObj.getClass().getSimpleName() + "的" + method.getName() + "方法");
>            Object o = method.invoke(srcObj, args);
>            System.out.println("日志：操作完成" + srcObj.getClass().getSimpleName() + "的" + method.getName() + "方法");
>            return o;
>        }
> }
> ```
...
>
> ```java
> BookService proxy = (BookService) ProxyUtil.createJdkProxy(BookService.class, new JdkLogAop(BookServiceImpl.class));
> proxy.add("窗边的小豆豆");
> proxy.edit(1, "窗边的小豆豆");
> proxy.del(1);
> ```
...
> ```java
> public class Fty {
>        public static Object createJdkProxy(Class c) {
>            return ProxyUtil.createJdkProxy(c.getInterfaces(), new JdkLogAop(c));
>        }
> 
>        public static Object createCglibProxy(Class c) {
>            return CglibUtil.createCglibProxy(c, new CglibLogAop());
>        }
> }
> ```
>
...
>
> ```java
> BookService jdkProxy = (BookService) Fty.createJdkProxy(BookServiceImpl.class);
> BookService cgProxy = (BookService) Fty.createCglibProxy(BookServiceImpl.class);
> ```
...
>概述：map 集合是一种键对应值的集合,键采用 set 集合方式存储，value采用 collection 集合方式存储
>
>- Interface Map<K,V>
...
[6]:data:image/png;base64,
