...
>
> ```typescript
> const genGetArray = (value: any, times: number = 5): any[] => {
...
>
> ```typescript
> console.log(genGetArray<number>(123, 4).map((item) => item.toFixed()));
> ```
>
...
> const genGetArray = <T, U>(param1: T, param2: U, tems: number): Array<[T, U]> => {
...
>
> ```typescript
> console.log(genGetArray(1, 'a', 3));
> ```
>
> - 返回结果：`[[1, "a"], [1, "a"], [1, "a"]]`
>
> ```typescript
> genGetArray(1, 'a', 3).forEach(item => {
>     console.log(...item)
>     // item[0].length // ERROR
...
> ```typescript
> // 定义函数类型
> let genGetArray: <T>(arg: T, times: number) => T[];
> // 实际函数体
> genGetArray = (arg: any, times: number) => {
...
> // 实际函数体
> const GetArray: GenGetArray = (arg: any, times: number) => {
>     return new Array(times).fill(arg)
> }
> ```
>
...
> // 实际函数体
> const genGetArray: GenGetArray = (arg: any, times: number) => {
>     return new Array(times).fill(arg)
> }
> ```
>
...
> **promise对象的特点**
>
> Promise对象代表一个异步操作，有三种状态：pending（进行中）、fulfilled（已成功）和rejected（已失败）。除了异步操作的结果，任何其他操作都无法改变这个状态
>
